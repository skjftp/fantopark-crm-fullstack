...
router.put('/:id/inclusions', authenticateToken, async (req, res) => {
  try {
    ...
    await createActivity(id, {
      ...
      description: 'Updated lead inclusions',
      user: req.user.email,
      metadata: {
        sections_updated: Object.keys(inclusionsData).filter(
          key => Array.isArray(inclusionsData[key]) && inclusionsData[key].length > 0
        )
      }
    });
...
// Helper function to create activity logs (if not already present)
async function createActivity(leadId, activityData) {
  try {
